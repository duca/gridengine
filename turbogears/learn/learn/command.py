...
# symbols which are imported by "from learn.command import *"
...
from learn.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("learn"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="learn.config")
...
def bootstrap():
    ...
    By default this runs the function 'learn.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-learn = learn.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="learn %s" % version)
    ...
    _read_config(args)
    from learn.model import bootstrap_model
...
def start():
    ...
    from learn.controllers import Root
